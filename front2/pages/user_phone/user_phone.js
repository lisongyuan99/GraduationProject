          ...
          success(res){
            if (res.confirm){
              bindingPhone({
                ...
                return app.Tips({ title: res.msg, icon: 'success' }, { tab: 5, url: '/pages/user_info/user_info' });
              }).catch(err => { return app.Tips({ title: err }); })
            } else if (res.cancel){
              return app.Tips({ title: '您已取消绑定！'}, { tab: 5, url: '/pages/user_info/user_info' });
            }
          }
        });
      }else
        return app.Tips({title:'绑定成功！',icon:'success'},{tab:5,url:'/pages/user_info/user_info'});
    }).catch(err=>{
